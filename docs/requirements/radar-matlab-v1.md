...
| RAD-V1-003 | The V1 maximum instrumented range shall be 100 km. The primary case shall contain a constant-RCS 10 m² target whose reported range is 100 km at the common five-PRF scan midpoint and whose radial speed is approximately 800 km/h. The usable-pulse count and timing schedule shall be measurable; WP4 selects and verifies it at G2, while downstream WP6e may reject it and reopen G2. 128 usable returns per PRF is not frozen. | accepted, G2 schedule pending | Seeded primary fixture; WP2 coverage study and WP4 evidence, with WP6e downstream review |
...
| RAD-V1-022 | The reported V1 range domain shall use a common five-PRF scan-midpoint reference epoch. The candidate range domain is 6.80–100.00 km with guarded processing support internally extending to 100.05 km for motion; values below 6.80 km are not claimed. The provisional G2 schedule is usable pulses `[22,25,28,32,35]` for PRFs `[1700,1900,2150,2450,2700]` Hz; WP4 shall verify it at G2, and downstream WP6e may reject it and reopen G2. The current arithmetic is 65.147760 ms usable dwell + 2.358240 ms priming + 2.849847 ms transitions = 70.355847 ms, leaving 0.297153 ms under the 70.653 ms cap. WP4 shall verify receive-window priming, transitions, narrowband-DDC filter state/delay, and near-range gating at G2. ADR 0016 and ADR 0017 record the schedule decision. | G2 schedule and DDC validation pending | Revised G1 study, ADR 0014, ADR 0016, ADR 0017, and WP4 timing contract; ADR 0011 is historical |
| RAD-V1-023 | The V1 ideal input assumption is a band-limited 45–55 MHz ADC input centered at the 50 MHz IF, with a 10 MHz complex chirp spanning -5 to +5 MHz and multistage decimation from a 150 MS/s real ADC through 50 MS/s complex (/3) to 12.5 MS/s complex (/4). A real-only /3 followed by IQ recovery is rejected because the IF aliases to DC. The MVP generator emits ADC-rate samples directly while retaining coherent phase derived from the exact ADR 0015 RF invariant. ENOB and jitter are downstream sensitivity studies, not pending V1 assumptions; finite-filter and alias rejection remain G2 validation items; no automatic SNR or Pd inference is permitted. | accepted ideal-input assumption; finite-filter/alias G2 validation pending | ADR 0012, ADR 0013, ADR 0014, ADR 0015, ADR 0017, revised G1 study, and WP4/WP5/WP6/WP8 validation |
