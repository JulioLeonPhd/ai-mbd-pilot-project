...
the rationale for consequential choices remains in the ADRs. WP2's bounded
simulation baseline is adopted by [ADR 0011](../adr/0011-adopt-v1-simulation-timing-baseline.md);
the remaining G2 and implementation decisions are identified below.
...
| RAD-V1-006 | An ideal mid-range, high-input-SNR fixture shall resolve two equal-RCS moving targets with shared nonzero radial velocity and 1 m range separation, producing two reports. | accepted | Range and end-to-end fixture |
...
| RAD-V1-022 | The reported V1 range domain shall use a common five-PRF scan-midpoint reference epoch. The G1 analytic baseline is 6.80–100.00 km with guarded processing support internally extending to 100.05 km for motion; values below 6.80 km are not claimed. WP4 shall verify 128 usable returns per PRF, receive-window priming, and transitions at G2; if that baseline is disproven, G1 shall reopen. | G1 baseline adopted, G2 schedule pending | WP2 continuous-range study, ADR 0011, and WP4 timing contract |
...
decision and updated evidence link. ADR 0009 remains in force except for the
V1 Pd/Pfa gate clarification in [ADR 0010](../adr/0010-v1-pd-pfa-demonstration-gate.md).
