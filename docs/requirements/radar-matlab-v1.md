...
| RAD-V1-022 | The reported V1 range domain shall use a common five-PRF scan-midpoint reference epoch. The candidate range domain is 6.80–100.00 km with guarded processing support internally extending to 100.05 km for motion; values below 6.80 km are not claimed. WP4/WP6e shall select and verify usable pulses per PRF, receive-window priming, transitions, and narrowband-DDC filter state/delay and near-range gating at G2. ADR 0016 records the five-PRF decision and reopened pulse-count rationale. | G2 schedule and DDC validation pending | Revised G1 study, ADR 0014, ADR 0016, and WP4 timing contract; ADR 0011 is historical |
